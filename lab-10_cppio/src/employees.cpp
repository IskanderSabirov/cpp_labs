...
/// Developer
    ...
    [[maybe_unused]] Developer::Developer(const std::string &name, int32_t base_salary, int32_t bonus) : Employee(name,
                                                                                                                  base_salary) {
        if (bonus != 0 && bonus != 1)
    ...
    void Developer::print(std::ofstream &ofstream) const {
        ofstream << mymanip::write_le_int32(1);
        ofstream << mymanip::write_string(name_);
        ofstream << mymanip::write_le_int32(base_salary_) << mymanip::write_bool(has_bonus_);
    }
...
/// Sales Manager
    ...
    [[maybe_unused]] SalesManager::SalesManager(const std::string &name, int32_t base_salary, int32_t s_items,
                                                int32_t i_price) : Employee(
            name, base_salary) {
    ...
    void SalesManager::print(std::ofstream &ofstream) const {
        ofstream << mymanip::write_le_int32(2);
        ofstream << mymanip::write_string(name_);
        ofstream << mymanip::write_le_int32(base_salary_) << mymanip::write_le_int32(sold_number_);
        ofstream << mymanip::write_le_int32(item_price_);
    }
...
/// friend functions
    ...
    std::ofstream &operator<<(std::ofstream &ofstream, const EmployeesArray &employeesArray) {
        ofstream << mymanip::write_le_int32(employeesArray.size_);
        for (int32_t i = 0; i < employeesArray.size_; i++)
    ...
    std::ifstream &operator>>(std::ifstream &ifstream, EmployeesArray &employeesArray) {
        int32_t number;
        ifstream >> mymanip::read_le_int32(&number);
